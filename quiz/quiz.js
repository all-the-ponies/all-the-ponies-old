import { getUrlParameter, LOC, setUrlParameter, toTitleCase, scrollIntoViewWithOffset } from "../scripts/common.js";
...
export default class Quiz extends Page {
    ...
    transformName(name) {
        return gameData.transformName(name, this.options)
    }
    ...
    checkName() {
        ...
        // let pony = gameData.matchName(this.nameInput.val())
        ...
        if (!(this.guessedPonies.includes(pony.id))) {
            ...
            scrollIntoViewWithOffset(
                nameElement[0],
                this.gameBar.height() + Number(getComputedStyle(this.ponyListElement[0]).marginTop.replace('px', '')),
            )
            this.nameInput.val('')
            this.updateProgress()
        }
